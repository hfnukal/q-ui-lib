...
- **`components/<name>/index.tsx`** (+ `meta.json` podle `CREATE.md`) — hlavní veřejné API dané položky.
...
- **Oddělení od komponent:** vlastní strom (např. `utilities/` nebo `lib/` — sjednotit s `package.json` → `exports`), aby bylo jasné: **utility = logika a tokeny bez vlastního UI stromu**, zatímco **`components/<name>/`** = kopírovatelný UI modul (+ jeho `meta.json`).
...
### `meta.json` u každé komponenty

Každá položka ve `components/<name>/` má **`meta.json`** (základní schéma viz `Q_UI_LIB.md`: `name`, `version`, `type`). Verze se používá při **`update`** v CLI k detekci odlišné verze v cílovém projektu.
...
Dokud pole v `meta.json` chybí, lze závislosti odvodit statickou analýzou importů v `index.tsx`; explicitní zápis v metadatech je čitelnější a stabilnější pro CLI.
...
| `qui update <cíl> [komponenty...]` | Buď synchronizuje **jménem uvedené** komponenty z knihovny do aplikace, nebo při absenci jmen projde **`meta.json`** a interaktivně nabídne aktualizaci při nesouladu verzí. |
| `qui sync-template <cíl>` | Zarovnání souborů aplikace se **`template/`** (diff / potvrzení, případně `--yes`). |
| `qui verify-meta [komponenty…]` *(plánováno)* | Ověří, že **`meta.json`** (`cssVariables`, `composition`, …) **souhlasí s implementací** v `index.tsx` (a případnými soubory ve složce), případně vypíše diff nebo nabídne doplnění metadat. |
...
1. Po přidání nebo aktualizaci komponenty **zkontrolovat závislosti** (z `meta.json` a/nebo z importů `~/components/ui/…` v zkopírovaném kódu).
...
4. **`verify-meta`** (až bude implementováno): porovnat deklarované **`cssVariables`** s výskyty v kódu a šabloně; ověřit **`composition`** vůči exportovaným podkomponentám / použitým headless dílům — při nesouladu **varovat** nebo **navrhnout** úpravu `meta.json`.
...
- **`template/`** drží minimální Qwik základ; **`demo/`** je plná ukázka s komponentami a routami. **`meta.json`** lze rozšířit o závislosti; CLI (`npm run qui`) má při přidávání/aktualizaci **doplňovat závislé komponenty** a **upozorňovat na npm balíčky**.
...
- Pojmenování **`Root` / Trigger / …** vs. jednoduchý `component$`, zápis **`cssVariables`** a **`composition`** v `meta.json` a plánovaná **verifikace v CLI** — viz **`BASE_COMPONENTS.md`**.
