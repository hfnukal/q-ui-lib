...
function parseLLLayout(text: string): { monthStart: number; yearStart: number } | null {
...
function segmentAtCursorLL(text: string, cursor: number): "day" | "month" | "year" | null {
...
function mapCursorToCanonical(draftLen: number, draftPos: number, canonicalLen: number): number {
  if (canonicalLen <= 0) return 0;
  if (draftLen <= 0) return 0;
  return Math.min(canonicalLen - 1, Math.round((draftPos / draftLen) * canonicalLen));
}
...
export default component$(() => {
  ...
  useTask$(({ track }) => {
    ...
    if (iso) {
      calendarViewMonth.value = dayjs(iso).startOf("month").format("YYYY-MM-DD");
    }
  });
    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-body text-secondary-label">
          ...
          <code class="rounded bg-fill-secondary/20 px-1 py-0.5 text-caption-1 text-label">dayjs</code>{" "}
          (výpočet mřížky, formát titulku). Styly odpovídají tokenům z COLORS.md; v demu je zapnutá
          locale <code class="text-caption-1">cs</code> pro názvy měsíců.
        </p>
      </div>
      ...
      <section class="space-y-3">
        ...
        <p class="text-callout text-secondary-label">
          <code class="text-caption-1">Calendar.Root</code> s vlastním rozložením (stejné podkomponenty
          jako u Panel).
        </p>
      ...
      <section class="space-y-3">
        ...
        <p class="text-callout text-secondary-label">
          <code class="text-caption-1">viewMonth</code> + <code class="text-caption-1">onViewMonthChange$</code>,{" "}
          omezení rozsahu <code class="text-caption-1">minDate</code> / <code class="text-caption-1">maxDate</code>{" "}
          (řetězce <code class="text-caption-1">YYYY-MM-DD</code>).
        </p>
        <CodeExample code={codeControlled}>
          <div class="flex flex-wrap items-start gap-6">
            <Calendar.Panel
            ...
            <div class="text-callout text-secondary-label">
              <p>
                Měsíc:{" "}
                <span class="font-medium text-label">{controlledView.value}</span>
              </p>
              <p>
                Den:{" "}
                <span class="font-medium text-label">
      ...
      <section class="space-y-3">
        ...
        <p class="text-callout text-secondary-label">
          Při platném zápisu se hned přepíše výběr v kalendáři a měsíční pohled. Neplatný text po opuštění pole
          zobrazí <code class="text-caption-1">Field.Error</code> a stále viditelnou nápovědu ve{" "}
          <code class="text-caption-1">Field.Description</code>. Šipky nahoru / dolů v poli mění podle pozice
          kurzoru den, měsíc nebo rok (u formátu LL).
        </p>
        <CodeExample code={codeInput}>
          <div
            ...
            onFocusOut$={$((e: FocusEvent) => {
              ...
              if (!raw) {
                ...
                calendarViewMonth.value = dayjs().startOf("month").format("YYYY-MM-DD");
              } else if (parsed) {
          ...
          >
            <Field.Root>
              ...
              <Input
                ...
                onFocusIn$={$(() => {
                  ...
                  if (inputPickerIso.value) {
                    inputDraft.value = isoToLocaleDisplay(inputPickerIso.value);
                  }
                })}
                onInput$={$((e) => {
                ...
                onKeyDown$={$((e) => {
                  ...
                  const isoNow =
                    parseLocaleDateToIso(inputDraft.value.trim()) ?? inputPickerIso.value;
                  if (!isoNow) return;
              ...
              />
              <Field.Description id={hintId}>{DATE_FORMAT_HINT}</Field.Description>
              {dateInputError.value ? (
                <Field.Error id={errId}>
                  Neplatné datum. Zkontrolujte zápis — platí formáty uvedené v nápovědě výše.
                </Field.Error>
              ) : null}
              <p class="text-caption-1 text-tertiary-label">
                ISO:{" "}
                <code class="text-caption-1 text-label">{inputPickerIso.value ?? "—"}</code>
              </p>
            </Field.Root>
            {pickerOpen.value ? (
