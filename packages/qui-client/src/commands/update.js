...
async function runUpdate(context) {
  ...
  try {
    ...
    const expanded = expandDependencies(componentsRootDir, orderedUilibs, seeds);
    const expandedKeys = expanded.map((component) => component.key);
    const items = [];
    const npmPackages = collectNpmDependencies(componentsRootDir, expandedKeys);
    const npmResult = await installMissingDependencies(cwd, npmPackages, flags, policy);
