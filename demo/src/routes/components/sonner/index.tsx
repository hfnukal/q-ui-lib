    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-sm text-slate-600">
          ...
          — bez React balíčku <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">sonner</code>. Stav drží{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Sonner.Toaster</code>, volání přes{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">useSonner()</code>           (tokeny z COLORS.md). Více zpráv je ve výchozím stavu naskládaných: nejnovější je nahoře v plné
          velikosti; u většího počtu karet se měřítko snižuje jen do 4. úrovně od přední, pak už ne.
          Ve sbaleném stavu jsou vidět jen 3 nejnovější zprávy (starší po najetí myší). Každá vrstva vzadu je
          o 30 % průhlednější než ta před ní. Po hoveru jsou všechny plně neprůhledné a rozložené pod sebe.
        </p>
      </div>

      <section class="space-y-3">
        <h2 class="text-lg font-medium text-slate-800">Typy a zprávy</h2>
        <CodeExample code={codeUsage} previewTabLabel="Ukázka" codeTabLabel="Kód">
