    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-body text-secondary-label">
          ...
          <code class="text-caption-1">Checkbox</code>. Pro štítek s kliknutím
          použij <code class="text-caption-1">CheckboxField</code> nebo vlastní{" "}
          <code class="text-caption-1">Label</code> s{" "}
          <code class="text-caption-1">onClick$</code> (atribut{" "}
          <code class="text-caption-1">for</code> u{" "}
          <code class="text-caption-1">div role="checkbox"</code> nestačí).
        </p>
      </div>
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            <code class="text-caption-1">Checkbox.Root</code> +{" "}
            <code class="text-caption-1">Checkbox.Indicator</code>.
          </Desc>
          <TabExample>
            <div class="flex flex-wrap items-center gap-4">
              <Checkbox.Root
                bind:checked={bound}
                aria-labelledby="demo-cb-compound-label"
              >
                <Checkbox.Indicator>
                  <CheckboxCheckIcon />
                </Checkbox.Indicator>
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            Jedna komponenta s ikonou; vždy s přímým headless stromem uvnitř
            (vhodné s <code class="text-caption-1">bind:checked</code>).
          </Desc>
          <TabExample>
            <div class="flex flex-wrap items-center gap-4">
              <CheckboxControl
                bind:checked={controlOnly}
                aria-label="Ukázkový souhlas"
              />
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            Kombinuje <code class="text-caption-1">CheckboxControl</code> a{" "}
            <code class="text-caption-1">Label</code>; klik na text přepne
            stejný signál jako checkbox.
          </Desc>
          <TabExample>
            <CheckboxField
              bind:checked={fieldAccepted}
          ...
          <TabCode>{codeField}</TabCode>
        </CodeExample>
      </section>

      <section class="space-y-3">
        <h2 class="text-headline text-label">
          Vlastní řádek (Label + onClick$)
        </h2>

        <CodeExample>
          <Desc>
            Stejné chování ručně:{" "}
            <code class="text-caption-1">aria-labelledby</code> + přepnutí
            signálu v <code class="text-caption-1">onClick$</code> na štítku
            (bez <code class="text-caption-1">for</code>).
          </Desc>
          <TabExample>
            <div class="flex items-center gap-2">
              <CheckboxControl
                bind:checked={manual}
                id="demo-manual-cb"
                aria-labelledby="demo-manual-lbl"
              />
              <Label
                id="demo-manual-lbl"
          ...
          <TabCode>{codeManualLabel}</TabCode>
        </CodeExample>
      </section>
    </div>
  );
