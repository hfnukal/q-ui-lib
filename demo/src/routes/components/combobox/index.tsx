    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-sm text-slate-600">
          ...
          Combobox — stejné tokeny jako Select (COLORS.md). Výchozí režim je{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
            popover
          </code>
          ; struktura odpovídá shadcn Combobox (
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
            Control
          </code>{" "}
          = vstup + tlačítko, seznam v{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
            Popover
          </code>
          ).
      ...
      <section class="space-y-3">
        <h2 class="text-lg font-medium text-slate-800">
          Filtrování a prázdný stav
        </h2>

        <CodeExample>
          <Desc>
            Headless ve výchozím stavu zapíná{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              filter
            </code>{" "}
            — při psaní se schovávají nevyhovující položky. S{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              Combobox.Empty
            </code>{" "}
            zobrazíš hlášku, když nic nezůstane.
          </Desc>
          <TabExample>
            <Combobox.Root
              class="w-full max-w-md"
              filter
              placeholder="Hledej framework…"
            >
              <Combobox.Label>Framework</Combobox.Label>
              <Combobox.Control>
                <Combobox.Input />
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            Vybraná hodnota v signálu přes{" "}
            <code class="text-caption-1">bind:value</code> (řízený combobox).
          </Desc>
          <TabExample>
            <div>
              <Combobox.Root
              ...
              <p class="mt-2 text-caption-1 text-secondary-label">
                Aktuální hodnota:{" "}
                <span class="font-medium text-label">{city.value}</span>
              </p>
            </div>
          </TabExample>
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            ...
            na kořeni — headless to v multi režimu nepovoluje). Vybrané položky
            vykresli jako{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              Combobox.Chip
            </code>{" "}
            ...
            ; odfiltrováním z pole se synchronizuje stav. Na{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              Combobox.Control
            </code>{" "}
            použij{" "}
            ...
            (žádná šipka): seznam otevři při fokusu vstupu přes{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              bind:open
            </code>{" "}
            a{" "}
            ...
            . Text ve vstupu drž v samostatném signálu přes{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              Combobox.Input bind:value
            </code>{" "}
            (výchozí prázdný řetězec) — headless by jinak při{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              multiple
            </code>{" "}
            vyplnil vstup názvy vybraných položek; výběr zůstává jen na chipích.
            Po změně výběru vyčisti filtr v{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              onChange$
            </code>{" "}
            na kořeni. V seznamu označ vybrané řádky přes{" "}
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              Combobox.ItemIndicator
            </code>{" "}
            (headless je zobrazí jen u vybraných položek).
          </Desc>
          <TabExample>
            <div>
              <Combobox.Root
          ...
          <TabCode>{codeMultiselect}</TabCode>
        </CodeExample>
      </section>
      ...
      <section class="space-y-3">
        ...
        <CodeExample>
          <Desc>
            ...
            mapuje na Floating UI umístění (
            <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">
              bottom-start
            </code>{" "}
            /{" "}
        ...
        <div class="max-w-md rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          >
            ...
            <Combobox.Description>
              Vyberte výchozí jazyk (filtrování vypnuto).
            </Combobox.Description>
            <Combobox.Control>
              <Combobox.Input />
              <Combobox.Trigger>{chevron}</Combobox.Trigger>
