    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-sm text-slate-600">
          ...
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Modal</code> (viz SHADCN.md: Dialog ≈ Modal) — vycentrované
          okno s backdropem, stejné díly jako Sheet. Ukázky používají{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">CodeExample layout=&quot;stack&quot;</code>, aby{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">{"<dialog>"}</code> nebyl v neaktivním tabpanelu. Animace a
          skrytí mimo <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">:modal</code> jsou v{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">global.css</code> (
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">q-dialog-panel</code>).
        </p>
      </div>
            ...
            <Dialog.Panel>
              ...
              <Dialog.Content>
                <p class="text-callout text-secondary-label">
                  ...
                  <code class="rounded bg-slate-100 px-1">closeOnBackdropClick=false</code>).
                </p>
              </Dialog.Content>
              <Dialog.Footer>
      ...
      <section class="space-y-3">
        ...
        <p class="max-w-prose text-sm text-slate-600">
          Na <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Dialog.Root</code> nastav{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">alert</code> — headless nastaví{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">role=&quot;alertdialog&quot;</code> a vypne zavření klikem na
          pozadí.
        </p>
        <CodeExample layout="stack" code={codeAlert} previewTabLabel="Ukázka" codeTabLabel="Kód">
          <Dialog.Root alert>
            <Dialog.Trigger>Smazat účet</Dialog.Trigger>
            <Dialog.Panel>
              <Dialog.Close class="absolute right-4 top-4 z-10" />
