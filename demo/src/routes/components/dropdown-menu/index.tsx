    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="mt-2 max-w-prose text-sm text-slate-600">
          Složené API{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">DropdownMenu.Root</code>,{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Trigger</code>,{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Popover</code> a položky nad{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">@qwik-ui/headless</code>{" "}
          <span class="font-medium">Dropdown</span> — stejný vzor jako shadcn Dropdown Menu; styly
          odpovídají tokenům z COLORS.md (jako Popover).
        </p>
      </div>
      ...
      <section class="space-y-3">
        ...
        <p class="max-w-prose text-sm text-slate-600">
          ...
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">CheckboxItem</code> s{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">bind:checked</code>; pro jednu
          hodnotu <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">RadioGroup</code> s{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">bind:value</code> a{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">RadioItem</code>.
        </p>
        <CodeExample code={codeCheckboxRadio} previewTabLabel="Ukázka" codeTabLabel="Kód">
          <MenuWithSelection />
        </CodeExample>
      </section>
      ...
      <section class="space-y-3">
        ...
        <p class="max-w-prose text-sm text-slate-600">
          Vnořené menu je druhý <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Dropdown</code>{" "}
          kontext: <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Sub</code>,{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">SubTrigger</code>,{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">SubContent</code>. Uvnitř panelu
          používej jen{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">SubItem</code> (případně{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">SubCheckboxItem</code> /{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">SubRadioItem</code>), ne hlavní{" "}
          <code class="rounded bg-slate-200/80 px-1 py-0.5 text-xs">Item</code> — jinak by headless
          walker sloučil indexy s hlavním menu.
        </p>
        <CodeExample code={codeSubmenu} previewTabLabel="Ukázka" codeTabLabel="Kód">
          <DropdownMenu.Root>
            <DropdownMenu.Trigger>Soubor</DropdownMenu.Trigger>
            <DropdownMenu.Popover gutter={4}>
              <DropdownMenu.Item>Nový</DropdownMenu.Item>
