 ...
 * @param {string} slug
 */
function isUiModuleForSlug(mod, slug) {
  return (
...
function isRedundantPrimaryComponentImport(imp, slug, componentName) {
  ...
  const mod = fromM[1];
  if (!isUiModuleForSlug(mod, slug)) return false;
...
function importBlockImportsNameFromUi(imp, slug, componentName) {
  ...
  if (!fromM || !isUiModuleForSlug(fromM[1], slug)) return false;
...
${tabExampleContent}
  ...
  const mainImportLine = mainImportNeeded
    ? `\nimport { ${componentName} } from "~/components/ui/base/${slug}";`
    : "";
