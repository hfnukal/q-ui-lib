...
export type ItemRootProps = Omit<PropsOf<"div">, "as"> &
  ...
  Partial<Pick<PropsOf<"button">, "type" | "disabled">> & {
    /** Renders a root element other than the default `span` (e.g. `a` for the whole row as a link). */
...
/**
 * Main row container (media, title, description, actions) in the shadcn/ui Item style.
 * The root tag can be changed via the `as` prop; the default is `span`, rendering runs through `Polymorphic`.
 * In text parents (`p`/`pre`) use `as="span"` and only phrasing content; for a block layout use `as="div"`.
 * For a link use `as="a"` and the usual anchor attributes (`href`, …).
 */
...
/** Group of items with unified spacing (shadcn `ItemGroup`). */
...
/** Left part of the row: icon, image, or avatar. */
...
/** Column with title and description; `min-w-0` for wrapping in a flex row. */
...
/** Right side of the row (buttons, arrow). */
...
/** Top block in the card (e.g. a full-width image). */
...
/** Bottom block below the content. */
